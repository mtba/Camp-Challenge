...
<body>
    ...
    //課題５ 確認ページからhidden情報を送ってページ遷移しているか
    if (isset($_POST["pass"]) && !empty($_SESSION['random']) ){

            if ($_SESSION['random']==$_POST["pass"] ) {     //入力データが乱数と一致するなら
                $name     = $_SESSION['name'];
                ...
                delete_session();   //セッション全破棄
                //これ以降このページに戻ってきても不正アクセス処理
                ?>
                ...
                <table>
                    ...
                    <tr>
                        <th>自己紹介:</th><td><?=nl2br($comment)?></td>
                    </tr>
                </table>
                <p>以上の内容で登録しました。</p>
                <?php
